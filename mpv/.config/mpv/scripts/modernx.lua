...
local tn_style = {
  ...
  closest_index  = (tn_style_format.closest_index):format(tn_palette.white, tn_palette.alpha_black, tn_palette.black,
    tn_palette.alpha_black, -1, -1),
  ...
  progress_text  = (tn_style_format.progress_text):format(tn_palette.white, tn_palette.black, tn_palette.alpha_opaque,
    tn_palette.alpha_black, 1, 1, 2, 2),
}
...
local function update_tn_osc_params(seek_y)
  local tn_state, tn_osc_stats, tn_osc, tn_style, tn_style_format = tn_state, tn_osc_stats, tn_osc, tn_style,
      tn_style_format
  ...
  if osc_changed then
    ...
    min(tn_osc.thumbnail.w, tn_osc.thumbnail.h) * 0.6667, tn_osc.font_scale.x, tn_osc.font_scale.y)
    tn_style.closest_index                   = (tn_style_format.closest_index):format(tn_palette.white,
      tn_palette.alpha_black, tn_palette.black, tn_palette.alpha_black, -1 * tn_osc.scale.x, -1 * tn_osc.scale.y)
    if tn_osc_stats.percent < 1 then
      tn_style.progress_text = (tn_style_format.progress_text):format(tn_palette.white, tn_palette.black,
        tn_palette.alpha_opaque, tn_palette.alpha_black, tn_osc.font_scale.x, tn_osc.font_scale.y, 2 * tn_osc.scale.x,
        2 * tn_osc.scale.y)
      tn_style.progress_mini = (tn_style_format.progress_mini):format(tn_palette.white, tn_palette.alpha_opaque,
        tn_osc.font_scale.x, tn_osc.font_scale.y)
    end
  end
  ...
  if (osc_changed or tn_osc.cursor.x_last ~= tn_osc.cursor.x) and tn_osc.osd.w_scaled >= (tn_osc.thumbnail.w + 2 * tn_osc.spacer.x) then
    ...
    tn_osc.thumbnail.left, tn_osc.thumbnail.right = tn_osc.position.x - tn_osc.thumbnail.w * 0.5,
        tn_osc.position.x + tn_osc.thumbnail.w * 0.5
...
local function show_thumbnail(seek_percent)
  ...
  local scale, thumbnail, total_expected, ready = tn_osc.scale, tn_osc.thumbnail, tn_osc_stats.total_expected,
      tn_osc_stats.ready
...
local draw_progress = {
  [message.queued]     = function(ass, index, block_w, block_h) ass:rect_cw((index - 1) * block_w, 0, index * block_w,
      block_h) end,
  [message.processing] = function(ass, index, block_w, block_h) ass:rect_cw((index - 1) * block_w, block_h * 0.2,
      index * block_w, block_h * 0.8) end,
  [message.failed]     = function(ass, index, block_w, block_h) ass:rect_cw((index - 1) * block_w, block_h * 0.4,
      index * block_w, block_h * 0.6) end,
}
...
local function display_tn_osc(seek_y, seek_percent, ass)
  ...
  local tn_osc_stats, tn_osc, tn_style, tn_style_format, ass_new, ass_rect, seek_percent = tn_osc_stats, tn_osc, tn_style,
      tn_style_format, ass_new, ass_rect, seek_percent * 0.01
  ...
  ass_rect(ass, -tn_osc.spacer.x, -tn_osc.spacer.top, tn_osc.thumbnail.w + tn_osc.spacer.x,
  ...
  ass_new(ass, tn_osc.position.x, tn_osc.thumbnail.top + tn_osc.thumbnail.h * 0.5, 5,
    tn_style.spinner ..
    (tn_style_format.spinner2):format(spinner_color, spinner_alpha, tn_osc.background.rotation * seek_percent * 1080),
  ...
  if tn_osc.display_progress.current ~= nil and not tn_osc.display_progress.current and tn_osc_stats.percent < 1 then
    ass_new(ass, tn_osc.mini.x, tn_osc.mini.y, 5,
  ...
  if tn_osc.display_progress.current then
    ...
    -- Text: Timer
    ass_new(ass, tn_osc.thumbnail.left + 3 * tn_osc.osc_scale.y, tn_osc.progress.mid, 4, tn_style.progress_text,
    ...
    ""
    ass_new(ass, tn_osc.position.x, tn_osc.progress.mid, 5, tn_style.progress_text,
      (tn_style_format.text_progress):format(temp and temp or 0, tn_osc_stats.total_expected) .. processing)

    -- Text: Percentage
    ass_new(ass, tn_osc.thumbnail.right - 3 * tn_osc.osc_scale.y, tn_osc.progress.mid, 6, tn_style.progress_text,
...
mp.register_script_message(message.osc.update, function(json)
  ...
  if new_data.osc_stats then
    ...
    tn_style_format.text_progress = tn_osc_stats.total > 99 and text_progress_format.three_digits or
    text_progress_format.two_digits
  ...
  if new_data.thumbnails and tn_state then
    ...
    for time_string, status in pairs(new_data.thumbnails) do
      ...
      tn_thumbnails_indexed[index] = ready and join_paths(tn_state.cache_dir, time_string) .. tn_state.cache_extension or
      status
        ...
        if mouse_hit(element) then
          ...
          if thumbfast.available then
            ...
            if osd_w and not thumbfast.disabled then
              ...
              elem_ass:draw_start()
              elem_ass:rect_cw(-thumbPad * r_h, -thumbPad * r_h, (thumbfast.width + thumbPad) * r_w,
